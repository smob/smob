...
<foaf:Document rdf:about="">
	...
	<foaf:primaryTopic rdf:resource="'.clean($this->object->_url).'"/>
	...
	<admin:generatorAgent rdf:resource="'.clean($this->_generator).'"/>
        ...
        if ($this->object) $rdf .= $this->object->getContent( $this );
        $rdf .= "\n</rdf:RDF>";
        return $rdf;
    }
}
...
class SIOCForum extends SIOCObject {
    ...
    function SIOCForum($id, $url, $page, $title='', $descr='', $type='sioc:Forum', $creator='', $admin='') {
        $this->_id = $id;
        $this->_url = $url;
        $this->_page = $page;
        $this->_posts = array();
...
class SIOCPost extends SIOCObject {
    ...
    function getContent( &$exp ) {
		...
		if($this->_has_part) {
            foreach($this->_has_part as $url=>$part) {
                $rdf .= "\t<dcterms:hasPart>\n";
				$rdf .= "\t\t<dcmitype:Image rdf:about=\"" . clean($part) . "\"/>\n";
				$rdf .= "\t</dcterms:hasPart>\n";
            }
        }
        if($this->_reply_of) {
